...
vSERVER = Tunnel.getInterface("tv")
            ...
            for k, v in pairs(Config['Objects']) do
                ...
                if DoesEntityExist(obj) then
                    ...
                    while #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(obj)) <= 2.0 do
                        ...
                        if playing then
                            drawtext('UTILIZE ~g~ /volume 0-10 ~w~PARA ALTERAR O VOLUME DO VIDEO',4,0.5,0.905,0.45,255,255,255,100)
                            drawtext('PRESSIONE ~g~ E ~w~PARA SINCRONIZAR O VIDEO & ~g~/desligar ~w~ para desativar a tv.',4,0.5,0.93,0.38,255,255,255,100)
                            ...
                            drawtext('DIGITE ~g~ /tv URL ~w~PARA DAR PLAY EM UM VIDEO',4,0.5,0.905,0.45,255,255,255,100)
                            drawtext('(pegar apenas o ID depois do = no link, apenas videos do youtube, Exemplo: ~g~/tv gwsaElRJI2M )',4,0.5,0.93,0.38,255,255,255,100)
...
RegisterCommand('tv', function(source, args)
    --if vSERVER.checkPermission() and args[1] then
    if args[1] then
        for k, v in pairs(Config['Objects']) do
...
RegisterCommand('desligar', function(src, args)
    if vSERVER.checkPermission() and args[1] then
    for k, v in pairs(data) do
        if #(GetEntityCoords(PlayerPedId()) - v['Coords']) <= 5.0 then
